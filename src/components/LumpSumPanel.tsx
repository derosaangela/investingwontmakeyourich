...
import { LumpSumInputs } from '@/types/calculator';

interface LumpSumPanelProps {
  inputs: LumpSumInputs;
  onInputChange: (inputs: LumpSumInputs) => void;
}

export function LumpSumPanel({ inputs, onInputChange }: LumpSumPanelProps) {
  const handleChange = (key: keyof LumpSumInputs, value: number | string) => {
    onInputChange({ ...inputs, [key]: value });
  };

  const handleNumericChange = (key: keyof LumpSumInputs, stringValue: string) => {
